...
function updateGame() {
  ...
  if (player.dx !== 0 || player.dy !== 0) {
    ...
    if (nextX >= 0 && nextX < GRID_COLS && nextY >= 0 && nextY < GRID_ROWS) {
      ...
      const enemyAtTarget = enemies.find((e) => e.x === nextX && e.y === nextY);
      if (enemyAtTarget) {
        // Check enemy type - Patroller always kills player
        if (enemyAtTarget.type === 'patroller') {
          console.log(`Player moved into patroller at (${nextX}, ${nextY})!`);
          loseLife();
          return; // Stop update if life lost
        } else if (isDrawing) {
          // Bouncer kills only when drawing
          console.log(`Player moved into bouncer at (${nextX}, ${nextY}) while drawing!`);
          loseLife();
          return; // Stop update if life lost
        } else {
          // Player is safe from bouncer if on captured ground
          // Stop player movement for this turn, but don't lose life
          console.log(`Player bumped into bouncer at (${nextX}, ${nextY}) while safe.`);
